...
"""
from ctypes import CDLL, c_char_p, c_size_t, c_void_p, create_string_buffer, byref, Structure, c_uint64, pointer
import time
...
class RadosPool(object):
    ...
    def get_xattr(self, key, xattr_name):
        ...
        ret = self.librados.rados_getxattr(self.pool, c_char_p(key), c_char_p(xattr_name), ret_buf, c_size_t(ret_length))
        if ret < 0:
    ...
    def set_xattr(self, key, xattr_name, xattr_value):
        ret = self.librados.rados_setxattr(self.pool, c_char_p(key), c_char_p(xattr_name), c_char_p(xattr_value), c_size_t(len(xattr_value)))
        if ret < 0:
